    ...
    execute: function () {
      ...
      _export("AutoGameBootstrap", AutoGameBootstrap = (_dec = ccclass('AutoGameBootstrap'), _dec(_class = executeInEditMode(_class = (_class2 = class AutoGameBootstrap extends Component {
        ...
        buildGame() {
          ...
          var audioManager = managers.addComponent(_crd && AudioManager === void 0 ? (_reportPossibleCrUseOfAudioManager({
            error: Error()
          }), AudioManager) : AudioManager);
          audioManager.audioSource = audioSource;
          audioManager.musicSource = musicSource;
          var uiManager = managers.addComponent(_crd && UIManager === void 0 ? (_reportPossibleCrUseOfUIManager({
            error: Error()
          }), UIManager) : UIManager);
          var tutorialManager = managers.addComponent(_crd && TutorialManager === void 0 ? (_reportPossibleCrUseOfTutorialManager({
            error: Error()
          }), TutorialManager) : TutorialManager);
          var safeAreaLayout = managers.addComponent(_crd && SafeAreaLayout === void 0 ? (_reportPossibleCrUseOfSafeAreaLayout({
            error: Error()
          }), SafeAreaLayout) : SafeAreaLayout);
          var scoreManager = managers.addComponent(_crd && ScoreManager === void 0 ? (_reportPossibleCrUseOfScoreManager({
            error: Error()
          }), ScoreManager) : ScoreManager);
          var timerManager = managers.addComponent(_crd && TimerManager === void 0 ? (_reportPossibleCrUseOfTimerManager({
            error: Error()
          }), TimerManager) : TimerManager);
          var comboManager = managers.addComponent(_crd && ComboManager === void 0 ? (_reportPossibleCrUseOfComboManager({
            error: Error()
          }), ComboManager) : ComboManager);
          var moleManager = managers.addComponent(_crd && MoleManager === void 0 ? (_reportPossibleCrUseOfMoleManager({
            error: Error()
          }), MoleManager) : MoleManager);
          var gameManager = managers.addComponent(_crd && GameManager === void 0 ? (_reportPossibleCrUseOfGameManager({
            error: Error()
          ...
          var resultLabels = this.buildResultPanel(resultPanel, gameManager, audioManager);
        ...
        buildHomePanel(homePanel, gameManager, audioManager) {
          ...
          var startButton = this.createButton('StartButton', '开始游戏', new Color(255, 184, 65, 255), () => {
        ...
        buildGamePanel(gamePanel, gameManager, moleManager) {
          ...
          var pauseButton = this.createButton('PauseButton', '暂停', new Color(122, 190, 91, 255), () => {
        ...
        buildResultPanel(resultPanel, gameManager, audioManager) {
          ...
          var replayButton = this.createButton('ReplayButton', '再来一次', new Color(255, 184, 65, 255), () => {
            gameManager.handleReplayButton();
          }, 340, 92, 36);
          var homeButton = this.createButton('HomeButton', '返回首页', new Color(122, 190, 91, 255), () => {
            gameManager.handleHomeButton();
          }, 340, 92, 36);
          var shareButton = this.createButton('ShareButton', '分享战绩', new Color(255, 211, 97, 255), () => {
            var _finalScoreNode$getCo, _finalScoreNode$getCo2;

            audioManager.playButtonClick();
            var score = this.parseScoreFromLabel((_finalScoreNode$getCo = (_finalScoreNode$getCo2 = finalScoreNode.getComponent(Label)) == null ? void 0 : _finalScoreNode$getCo2.string) != null ? _finalScoreNode$getCo : '');
            (_crd && PlatformAdapter === void 0 ? (_reportPossibleCrUseOfPlatformAdapter({
              error: Error()
            }), PlatformAdapter) : PlatformAdapter).shareScore(score);
          }, 250, 78, 30);
          var leaderboardButton = this.createButton('LeaderboardButton', '排行榜', new Color(141, 205, 255, 255), () => {
            audioManager.playButtonClick();
            (_crd && PlatformAdapter === void 0 ? (_reportPossibleCrUseOfPlatformAdapter({
              error: Error()
        ...
        buildTutorialPanel(tutorialPanel, tutorialManager) {
          ...
          var actionButton = this.createButton('TutorialActionButton', '下一步', new Color(255, 184, 65, 255), () => {
            tutorialManager.handleNext();
          }, 330, 92, 34);
          var skipButton = this.createButton('TutorialSkipButton', '跳过引导', new Color(183, 215, 145, 255), () => {
        ...
        createLabel(name, text, fontSize, color, width, height) {
          if (width === void 0) {
            width = 360;
          }
        ...
        createButton(name, text, color, onClick, width, height, fontSize) {
          if (width === void 0) {
            width = 300;
          }
        ...
        createSoundButton(name, audioManager) {
          var soundButton = this.createButton(name, audioManager.isSoundEnabled() ? '声音：开' : '声音：关', new Color(255, 232, 149, 255), () => {
        ...
        clearGeneratedNodes() {
          var generatedNames = new Set(['Background', 'HomePanel', 'GamePanel', 'ResultPanel', 'PauseMask', 'EffectsLayer', 'TutorialPanel', 'Managers']);
