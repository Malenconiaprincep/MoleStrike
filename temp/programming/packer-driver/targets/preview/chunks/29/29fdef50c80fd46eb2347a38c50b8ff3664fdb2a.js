      ...
      _export("MoleManager", MoleManager = (_dec = ccclass('MoleManager'), _dec2 = property({
        ...
        constructor() {
          ...
          this.spawnOneMole = () => {
            ...
            var hole = this.holes[Math.floor(Math.random() * this.holes.length)];
            var config = this.pickMoleConfig();
            var moleNode = this.getMoleNode(config.type);
            hole.addChild(moleNode);
            moleNode.active = true;
            moleNode.setPosition(Vec3.ZERO);
            moleNode.layer = hole.layer;

            for (var child of moleNode.children) {
              child.layer = hole.layer;
            }

            var mole = moleNode.getComponent(_crd && Mole === void 0 ? (_reportPossibleCrUseOfMole({
              error: Error()
            }), Mole) : Mole);
        ...
        createDefaultMoleNode(type) {
          ...
          var eyeColor = type === (_crd && MoleType === void 0 ? (_reportPossibleCrUseOfMoleType({
            error: Error()
