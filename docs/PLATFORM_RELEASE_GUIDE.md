...
- 分享入口：结算页 `分享战绩`
- 排行榜入口：结算页 `排行榜`
- 抖音侧边栏复访：首页 `侧边栏` 按钮调用 `tt.navigateToScene({ scene: 'sidebar' })`
- 抖音添加到桌面：首页 `加桌面` 按钮调用 `tt.addShortcut()`
...
5. 在抖音开发者工具里测试：
   ...
   - 震动反馈
   - 首页 `侧边栏` 按钮能调起侧边栏
   - 首页 `加桌面` 按钮在支持机型上能触发添加桌面
   - 真机预览

## 抖音能力检测说明

- 侧边栏复访：已接入，入口在首页左上角，必须通过用户点击触发。
- 添加到桌面：已接入，入口在首页左上角；该能力主要影响广告金优惠，不是核心玩法。
- 订阅消息：需要先在抖音后台创建订阅模板并获得模板 ID，拿到 `tmplIds` 后再接 `tt.requestSubscribeMessage`。
- 插屏广告/激励视频广告：需要先在抖音后台开通广告能力并创建广告位 ID，拿到广告位 ID 后再接入。
- 内购能力：需要游戏具备相应版号/支付资质后再接入，当前版本不包含付费内容。
